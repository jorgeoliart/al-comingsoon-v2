 ...
 * @return {string}
 */
const injectInstance = (content, vue) => {
  let importContent = ''
  let injectContent = ''
  if (vue === 2) {
    //vue2注入
    ...
    if (content.indexOf('import i18n from') === -1) {
      injectContent += `
...
`
  ...
  let [lastImport] = content.match(/import(?!from).+from(?!from).+;?/gm) ? content.match(/import(?!from).+from(?!from).+;?/gm).reverse() : ['']
 ...
 * @return {string}
 */
exports.generateScript = (messages, content, needReplace, vue = 2) => {
  // 忽略js的抓取
  let ignores = {}
  let ignoreIndex = 0
  // 替换忽略的部分
  ...
  if (needReplace && hasReplace) {
    content = injectInstance(content, vue)
  }
  return content
}
