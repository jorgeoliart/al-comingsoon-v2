...
const getAllFiles = (exports.getAllFiles = (dir) => {
  ...
  fs.readdirSync(dir).forEach((item) => {
    ...
    if (fs.lstatSync(item).isDirectory()) {
      ...
      if (['.vue', '.js', '.jsx', '.ts', '.tsx'].includes(path.extname(item).toLowerCase())) {
        results.push(item)
      }
    }
  })
...
const grammar = (zh) => {
  if (zh.length < 8) {
    if (zh.indexOf('请') === 0 || zh.indexOf('不能') > -1 || zh.indexOf('必须为') > -1 || zh.indexOf('确定') === 0 || zh.indexOf('以内') > -1) {
      return false
    }
    return true
  }
...
// 英语内介词的转化 和 html标签的转化
const preposition = (str) => {
  let val = str
  val = val.replace(/ Of /g, ' of ')
  val = val.replace(/ To /g, ' to ')
  val = val.replace(/ And /g, ' and ')
  val = val.replace(/ For /g, ' for ')
  val = val.replace(/ Or /g, ' or ')
  // html标签的转化
  val = val.replace(/< span /g, '<span ')
  val = val.replace(/< \/ span >/g, '</span>')
  val = val.replace(/< \/ span/g, '</span')
  val = val.replace(/span >/g, 'span>')
...
// 英文大小写转化匹配规则
exports.replaceStr = function (zh, str) {
  let value = str
  // 全部转化
  if (grammar(zh)) {
    let arr = str.split(' ')
